...
# עיצוב CSS למראה אינסטגרם + ריבוע הוספה 1:1 רספונסיבי
st.markdown("""
<style>
    /* קרוסלה */
    ...
    /* הכפתור של הוספת מטבע - ריבוע 1:1 עם פלוס במרכז */
    ...
    div[data-testid="stPopover"] > button {
        ...
        font-size: 60px !important; /* גודל הפלוס */
        color: #bbb !important;
    ...
    /* הסתרת החץ הקטן של המערכת בתוך הכפתור */
...
DB_FILE = 'final_coins_db_v10.csv'
...
with tab1:
    ...
    # 2. ריבוע ה-"➕" הגדול בסוף הרשימה
    last_col_idx = len(filtered_df) % grid_size
    with cols[last_col_idx]:
        # ה-Popover כאן מעוצב ב-CSS להיות ריבוע 1:1 עם פלוס
        with st.popover("＋"):
        ...
        st.write("<p style='text-align:center; color:#999; font-weight:bold; margin-top:-5px;'>הוסף חדש</p>", unsafe_allow_html=True)
